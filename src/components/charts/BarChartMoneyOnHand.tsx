...
export interface MoneyOnHandDatum {
  range: string;
  count: number;
}

export function BarChartMoneyOnHand({ data }: { data: MoneyOnHandDatum[] }) {
  return (
    <motion.div
    ...
    >
      <Card className="bg-gradient-to-br from-gray-800 to-gray-900 border-gray-700 shadow-lg hover:shadow-xl transition-all duration-300">
        <CardHeader>
          <CardTitle className="text-[#F3F4F6] text-lg">Money on Hand Distribution</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={data}>
                ...
                <XAxis dataKey="range" stroke="#F3F4F6" tick={{ fill: '#F3F4F6', fontSize: 12 }} axisLine={{ stroke: '#374151' }} />
